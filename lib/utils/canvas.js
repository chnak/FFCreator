...
const { createCanvas, createImageData, registerFont } = require('../adapters/fabric-adapter');
// const Canvas = Utils.nodeRequire('canvas');
const WebGL2D = require('./webgl-2d');
...
const CanvasUtil = {
  ...
  toBuffer({ type, canvas, quality = 70 }) {
    ...
    switch (type) {
      ...
      case 'canvas':
        ...
        buffer = ctx.getImageData(0, 0, canvas.width, canvas.height).data;
        break;
  ...
  setFont(font, setFontFunc) {
    // if (FS.isFont(font)) {
      ...
      if (!Utils.storage[fontFamily]) {
        if (!isBrowser) registerFont(font, { family: fontFamily });
  ...
  getContext(canvas) {
    return canvas.getContext('2d') || WebGL2D.getContext(canvas);
  },

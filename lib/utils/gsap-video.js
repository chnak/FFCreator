...
const rgbHex = require('node-rgb-to-hex');
const fs = require('fs');
 ...
 */
const animationDurationSeconds = (timeline,fallbackDuration) => {
  let _eval = false;
  try { _eval = eval(timeline); } catch { }
  // eslint-disable-next-line no-undef
  const _tl = timeline === 'gsap' ? gsap.globalTimeline : _eval;
  if (!_tl) return fallbackDuration || { error: 'No timeline found.' };
 ...
 */
const animationDurationFrames = (timeline, fps,fallbackDuration) => {
  let _eval = false;
  try { _eval = eval(timeline); } catch { }
  // eslint-disable-next-line no-undef
  const _tl = timeline === 'gsap' ? gsap.globalTimeline : _eval;
  if (!_tl) {
    if (fallbackDuration) return Math.ceil(fallbackDuration * fps);
...
const discoverTimeline = (timeline) => {
  ...
  // eslint-disable-next-line no-undef
  const _tl = timeline === 'gsap' ? gsap.globalTimeline : _eval;
  if (_tl) return "gsap"; // 返回 'gsap' 表示使用 GSAP
  if (window.timeweb) return "timeweb"; // 返回 'timeweb' 表示使用 Timeweb
  return false;
};
...
const generateHTML=function(elementsHTML,conf){
  return `
      <!DOCTYPE html>
    <html>
    <head>
...
const videoExport = async (options) => {
  ...
  log(`${options.url}\n`, options.verbose);
  options.duration = options.duration || null; // 添加这行
  ...
  if (options.headless) {
    browser = await puppeteer.launch({ executablePath, headless: true, defaultViewport: null, args: ['--disable-blink-features=AutomationControlled', '--no-sandbox', '--allow-file-access-from-files', '--kiosk','--disable-web-security','--disable-features=SameSiteByDefaultCookies'] });
  } else {
    browser = await puppeteer.launch({ executablePath, headless: false, defaultViewport: null, args: ['--disable-blink-features=AutomationControlled', '--no-sandbox', '--allow-file-access-from-files','--disable-web-security','--disable-features=SameSiteByDefaultCookies'] });
  }
  ...
  try {
    if(options.url){
      ...
      const html=generateHTML(options.html)
  ...
  let durationSeconds = await page.evaluate(animationDurationSeconds, options.timeline, options.duration);
  
  let duration = await page.evaluate(animationDurationFrames, options.timeline, options.fps, options.duration);

  const isLoop=durationSeconds>3600?true:false;
  if(options.duration){
    durationSeconds=durationSeconds > 3600?options.duration:durationSeconds;
    const frames=options.duration*options.fps
    duration=duration>frames?frames:duration
  }
  ...
  if (options.advance === 'timeweb' || isLoop) {
  ...
  for (let x = startFrame; x < endFrame; x++) {
    ...
    if (options.advance === 'gsap' && !isLoop) {

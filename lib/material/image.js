...
const { utils, createCanvas, createImageData } = require('../adapters/fabric-adapter');
  ...
  drawCanvas(img, width, height) {
    ...
    if (this.conf.blur && this.creator) {
      ...
      if (blur > 0) {
        ...
        this.canvasContext.filter = `blur(${blur}px)`;

        StackBlur.imageDataRGBA(img, 0, 0, width, height, blur);
      }
    }

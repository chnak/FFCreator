...
const { ProxyObj, Text } = require('../adapters/fabric-adapter');
...
class FFText extends FFNode {
  ...
  async preProcessing() {
    ...
    if (!isBrowser) {
      // 跳过对系统字体的检查
      const systemFonts = ['sans-serif', 'serif', 'monospace', 'Arial', 'Helvetica', 'Times New Roman', 'Courier New'];
      const fontFamily = this.useFontFamily || 'sans-serif';
      if (!systemFonts.includes(fontFamily) && !fontFamily.startsWith('http') && !fs.existsSync(fontFamily)) {
        throw new Error(`Font not exists: ${fontFamily}`);
      }
      try {
        if (!systemFonts.includes(fontFamily)) {
          await this.setFont(fontFamily);
        }
      } catch (e) {
        if (!fontFamily.startsWith('http') && fs.existsSync(fontFamily)) {
          fs.unlinkSync(fontFamily);
          throw new Error(`Set font fail: ${fontFamily}`);
        }
      }
      this.updateStyle();

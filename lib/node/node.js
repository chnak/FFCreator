...
const { DisplayObject, BLEND_MODES } = require('../adapters/fabric-adapter');
...
class FFNode extends FFClip {
  ...
  updateAnimations() {
    ...
    if (typeof(this.conf.animate) === 'object') {
      this.addAnimate(this.conf.animate);
    }
    if (this.conf.effect) {
      if(Array.isArray(this.conf.effect)){
        this.conf.effect.map(item=>{
          this.addEffect(item.name, item.time, item.delay);
        })
      }else{
  ...
  setAnchor(anchorX, anchorY) {
    ...
    this.conf.anchor = [anchorX, anchorY];
  }
  ...
  setRotate(rotation = 0) {
    ...
    this.display.rotation = this.conf.rotate = rotation;
  }
  ...
  set opacity(opacity) {
    if (!this.display) return;
    this.display.alpha = this.conf.opacity = opacity;
  }
